...
// ?Ä?Ä Types matching the actual backend response ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
const ENTITY_TYPES = [
  ...
  { value: "application", label: "?âÁî®?¥ÊôØ" },
  { value: "category",    label: "?ÜÂ??ÜÈ?" },
] as const;

type EntityType = typeof ENTITY_TYPES[number]["value"];

const PERIOD_OPTIONS = [
  { value: 7,  label: "?ÄËø?7 Â§? },
  { value: 30, label: "?ÄËø?30 Â§? },
  { value: 90, label: "?ÄËø?90 Â§? },
];
...
function scoreColor(score: number) {
  if (score >= 70) return { text: "text-green-600", bar: "bg-green-500", label: "?ØÂ•Ω" };
  if (score >= 50) return { text: "text-yellow-600", bar: "bg-yellow-400", label: "?ÆÈÄ? };
  return { text: "text-red-500", bar: "bg-red-400", label: "?Ä?πÂ?" };
}
...
export default function ContentOptimizerPage() {
  ...
  // ?Ä?Ä Load entity list when type changes ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const d = await r.json();
      if (!r.ok) throw new Error(d.detail ?? "?ÜÊ?Â§±Ê?");
      setResult(d);
    } catch (e: unknown) {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold tracking-tight">AI ?ßÂÆπ?™Â?</h1>
        <p className="mt-0.5 text-sm text-muted-foreground">
          ?∏Ê??ÜÂ??ÅÊ??®Â†¥?ØÊ??ÜÈ?ÔºåAI ÁµêÂ?ÂØ¶È?ÊµÅÈ??∏Ê??ÜÊ??ÅÈù¢?•Â∫∑Â∫¶‰∏¶?ê‰??πÂ?Âª∫Ë≠∞
        </p>
      </div>

      <div className="grid gap-6 lg:grid-cols-[360px_1fr]">
        {/* ?Ä?Ä Â∑¶Ê?ÔºöÈÅ∏?áË®≠ÂÆ??Ä?Ä */}
        <div className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="text-base">?ÜÊ?Ë®≠Â?</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              {/* Entity Type */}
              <div>
                <label className="mb-1.5 block text-sm font-medium">?ßÂÆπÈ°ûÂ?</label>
              ...
              <div>
                <label className="mb-1.5 block text-sm font-medium">
                  ?∏Ê?{ENTITY_TYPES.find(t => t.value === entityType)?.label}
                ...
                >
                  <option value="">{loadingEntities ? "ËºâÂÖ•‰∏≠‚Ä? : `???∏Ê?${ENTITY_TYPES.find(t => t.value === entityType)?.label} ?î`}</option>
                  {entities.map(e => (
                    <option key={e.id} value={e.id}>{e.name}</option>
                  ))}
                </select>
                {entities.length === 0 && !loadingEntities && (
                  <p className="mt-1 text-xs text-muted-foreground">Ê≠§È??ãÁõÆ?çÁÑ°?ØÂ??êÁ??ßÂÆπ</p>
                )}
              </div>

              {/* Period */}
              <div>
                <label className="mb-1.5 block text-sm font-medium">?ÜÊ??ÇÈ?ÁØÑÂ?</label>
                <select
                  value={periodDays}
              ...
              <div className="flex gap-2 pt-1">
                <Button onClick={analyze} disabled={loading || !entityId} className="flex-1">
                  ...
                  {loading ? "?ÜÊ?‰∏≠‚Ä? : "AI ?™Â??ÜÊ?"}
                </Button>
                {result && (
                  <Button variant="outline" onClick={reset} title="Ê∏ÖÈô§ÁµêÊ?">
          ...
          {selectedEntity && result && (
            <Card>
              <CardContent className="pt-4">
                <p className="text-xs font-medium text-muted-foreground mb-1">?ÜÊ?Â∞çË±°</p>
                <p className="font-semibold">{selectedEntity.name}</p>
                <p className="text-xs text-muted-foreground mt-0.5 capitalize">{entityType} ¬∑ ?ÄËø?{periodDays} Â§?/p>
              </CardContent>
            </Card>
          )}
        </div>
          ...
          {!result && !loading && !error && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-sm font-medium text-muted-foreground">?∏Ê??ßÂÆπÂæåÈ??ä„ÄåAI ?™Â??ÜÊ???/p>
                <p className="mt-1 text-xs text-muted-foreground">
                  AI Â∞áÁ??àÂØ¶?õÊ??è„ÄÅ‰?ËºâÁ??ÅRFQ ËΩâÊ??áÂ??êÈ??¢ÂÅ•Â∫∑Â∫¶
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {loading && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-sm text-muted-foreground">AI Ê≠?ú®?ÜÊ?ÊµÅÈ??∏Ê??áÂÖßÂÆπÔ?Ë´ãÁ??ô‚Ä?/p>
              </CardContent>
            </Card>
          )}
          ...
          {result && (
            <div className="space-y-4">
              ...
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-end justify-between mb-3">
                    <div>
                      <p className="text-sm text-muted-foreground">?¥È??ßÂÆπ?•Â∫∑?ÜÊï∏</p>
              ...
              {result.priority_actions?.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm text-orange-600">???™Â?Ë°åÂ?ÔºàÁ??≥Ë??ÜÔ?</CardTitle>
                  </CardHeader>
                  <CardContent>
              ...
              {(result.revised_title || result.revised_description) && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">AI Âª∫Ë≠∞?πÂØ´?áÊ?</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    {result.revised_title && (
                      <div>
                        <p className="mb-1 text-xs font-semibold text-muted-foreground uppercase tracking-wide">Ê®ôÈ?Âª∫Ë≠∞</p>
                        <div className="flex items-start justify-between gap-2 rounded-md bg-muted/40 px-3 py-2">
                          ...
                          <Button variant="ghost" size="sm" className="h-7 w-16 shrink-0" onClick={() => copy(result.revised_title, "title")}>
                            {copied === "title" ? <><Check className="mr-1 h-3.5 w-3.5 text-green-500" /><span className="text-xs text-green-500">??/span></> : <><Copy className="mr-1 h-3.5 w-3.5" /><span className="text-xs">Ë§áË£Ω</span></>}
                          </Button>
                        </div>
                      </div>
                    )}
                    {result.revised_description && (
                      <div>
                        <p className="mb-1 text-xs font-semibold text-muted-foreground uppercase tracking-wide">?èËø∞Âª∫Ë≠∞</p>
                        <div className="flex items-start justify-between gap-2 rounded-md bg-muted/40 px-3 py-2">
                          ...
                          <Button variant="ghost" size="sm" className="h-7 w-16 shrink-0" onClick={() => copy(result.revised_description, "desc")}>
                            {copied === "desc" ? <><Check className="mr-1 h-3.5 w-3.5 text-green-500" /><span className="text-xs text-green-500">??/span></> : <><Copy className="mr-1 h-3.5 w-3.5" /><span className="text-xs">Ë§áË£Ω</span></>}
                          </Button>
                        </div>
                      </div>
                    )}
              ...
              {result.issues?.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">?ºÁèæ?èÈ?Ôºà{result.issues.length}Ôº?/CardTitle>
                  </CardHeader>
                  <CardContent>
              ...
              {result.suggestions?.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">?πÂ?Âª∫Ë≠∞ÔºàÊ??™Â?Á¥öÊ?Â∫èÔ?</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      {[...result.suggestions]
                        ...
                        .map((s, i) => (
                          <div key={i} className="flex items-start gap-3 rounded-md bg-muted/30 px-3 py-2 text-sm">
                            ...
                            <span className={`text-xs font-medium shrink-0 ${IMPACT_STYLES[s.expected_impact] ?? "text-muted-foreground"}`}>
                              {s.expected_impact === "high" ? "È´òÊ??? : s.expected_impact === "medium" ? "‰∏≠Ê??? : "‰ΩéÊ???}
                            </span>
                          </div>
                        ))}
                    </div>
              ...
              {result.content_gaps?.length > 0 && (
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">?ßÂÆπÁº∫Âè£ÔºàÂª∫Ë≠∞Ë??ÖÁ?‰∏ªÈ?Ôº?/CardTitle>
                  </CardHeader>
                  <CardContent>
