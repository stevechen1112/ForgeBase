...
export default function BriefPreviewPage() {
  ...
  const handleGenerate = async () => {
    if (!confirm("觸發 AI 生成將消耗 API 額度，確認繼續？")) return;
    setGenerating(true);
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <Link href="/dashboard/briefs" className="text-xs text-muted-foreground hover:underline">← 所有摘要</Link>
          </div>
          <h1 className="text-2xl font-semibold text-foreground">
            AI 預覽：{brief.title_draft ?? brief.target_page_type}
          </h1>
          <div className="mt-1 flex items-center gap-3 text-sm text-muted-foreground">
            <span>頁型：<strong>{brief.target_page_type}</strong></span>
            <StatusBadge
      ...
      {logs.length === 0 ? (
        <div className="rounded-xl border-2 border-dashed border-gray-200 py-16 text-center text-muted-foreground">
          ...
          <p className="text-sm mt-1">點擊「觸發 AI 重新生成」開始生成</p>
        </div>
      ) : (
        <>
          {/* Log tabs */}
            ...
            <div className="space-y-4">
              ...
              {!!(output.seo_title || output.seo_description) && (
                <div className="rounded-xl border border-blue-100 bg-blue-50 p-5">
                  <h2 className="text-sm font-semibold text-blue-700 mb-2">SEO</h2>
                  {!!output.seo_title && (
