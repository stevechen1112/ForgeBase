...
import { RefreshCw, TrendingUp, Clock, Filter, FileText, Lightbulb } from "lucide-react";
...
type Outcomes = {
  ...
  first_response: { avg_hours: number | null; sla_rate_pct: number | null; responded: number };
...
const STATUS_LABEL: Record<string, string> = {
  ...
  negotiation: "談判中", won: "成交", lost: "流失", expired: "過期",
};
...
export default function OutcomesPage() {
  ...
  const qualifiedDelta = outcomes
    ...
    : 0;
  const maxFunnelCount = funnel ? Math.max(...funnel.layers.map((l) => l.count), 1) : 1;
    ...
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-5">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            ...
            <p className="mt-2 text-3xl font-bold">
              {outcomes?.first_response.avg_hours != null ? `${outcomes.first_response.avg_hours}h` : "—"}
            </p>
            <p className="text-xs text-muted-foreground">已回覆 {outcomes?.first_response.responded ?? 0} 件</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4">
            ...
            <p className="text-xs text-muted-foreground">買家時區工作時間計時</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-4">
        ...
        <Card>
          <CardContent className="pt-4 pb-4">
            ...
            <p className="mt-2 text-3xl font-bold">{outcomes?.top_source_pages.length ?? 0}</p>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <Filter className="h-4 w-4 text-primary" />業務漏斗（近 {funnel?.days ?? 30} 天）
            </CardTitle>
          </CardHeader>
              ...
              <div key={layer.layer} className="flex items-center gap-3">
                ...
                <div className="h-5 flex-1 overflow-hidden rounded bg-muted">
                  <div
                    ...
                    style={{ width: `${Math.max(2, (layer.count / maxFunnelCount) * 100)}%` }}
                  />
                </div>
                <span className="w-14 text-right text-sm font-bold">{layer.count}</span>
                <span className="w-16 text-right text-xs text-muted-foreground">
                  {layer.conversion_from_prev_pct != null ? `${layer.conversion_from_prev_pct}%` : "—"}
                </span>
              </div>
            ))}
            {funnel?.bottleneck_layer && (
              <p className="pt-2 text-xs text-red-500">
                瓶頸層：{funnel.layers.find((l) => l.layer === funnel.bottleneck_layer)?.label} — 優先改善此層轉化
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <TrendingUp className="h-4 w-4 text-primary" />RFQ 狀態漏斗（目前快照）
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {STATUS_ORDER.map((s) => (
                <div key={s} className="rounded-lg border px-3 py-2 text-center">
                  <p className="text-lg font-bold">{outcomes?.funnel_status[s] ?? 0}</p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <FileText className="h-4 w-4 text-primary" />本月 RFQ 來源頁 Top 5
            </CardTitle>
          </CardHeader>
          <CardContent>
            {!outcomes?.top_source_pages.length ? (
              <p className="py-6 text-center text-sm text-muted-foreground">本月尚無帶來源的 RFQ</p>
            ) : (
              <table className="w-full text-sm">
                <tbody className="divide-y">
                  {outcomes.top_source_pages.map((s) => (
                    <tr key={s.source_page}>
                      <td className="max-w-[320px] truncate py-2 pr-3 font-mono text-xs">{s.source_page}</td>
                      <td className="py-2 text-right"><Badge variant="secondary">{s.rfq_count} 件</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base">
              <Lightbulb className="h-4 w-4 text-amber-500" />下週建議
            </CardTitle>
          </CardHeader>
          <CardContent>
