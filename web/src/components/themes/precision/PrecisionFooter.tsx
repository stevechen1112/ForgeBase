...
type FooterMessages = {
  ...
  footerCta: { requestQuote: string };
};

export async function PrecisionFooter({ siteConfig }: { siteConfig: SiteConfig }) {
  const [locale, copy] = await Promise.all([
    getLocale(),
    getMessageNamespace<FooterMessages>("footer"),
  ]);
  ...
  return (
    <footer className="border-t border-white/10 bg-[#080c0e] text-gray-400">
      <div className="mx-auto grid max-w-[1440px] gap-10 px-6 py-12 md:grid-cols-[1.2fr_1fr_auto] lg:px-10">
        ...
        <Link href={localizedPath(locale, "/rfq")} className="inline-flex h-fit items-center gap-2 border border-lime-300 px-5 py-3 text-xs font-black uppercase tracking-wider text-lime-300 hover:bg-lime-300 hover:text-black">{copy.footerCta.requestQuote} <ArrowUpRight size={15}/></Link>
      </div>
      <div className="border-t border-white/10 px-6 py-5 text-center text-[10px] uppercase tracking-[0.18em] text-gray-600">{copy.builtWithPrecision}</div>
    </footer>
  );
}
