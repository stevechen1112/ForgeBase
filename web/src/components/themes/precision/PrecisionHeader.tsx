  ...
  return (
    <header className="sticky top-0 z-40 border-b border-white/10 bg-[#0b1013]/95 text-white backdrop-blur">
      <div className="mx-auto flex h-20 max-w-[1440px] items-center justify-between px-5 lg:px-10">
        <NextLink href={localizedPath(locale, "/")} className="flex items-center gap-3" aria-label={`${siteConfig.brandName} home`}>
          {siteConfig.logoUrl ? (
          ...
          <span className="text-base font-black uppercase tracking-[0.12em]">{siteConfig.brandName}<small className="block text-[9px] font-semibold tracking-[0.28em] text-gray-500">Precision / Demo</small></span>
        </NextLink>
      ...
      {open && <nav className="grid border-t border-white/10 bg-[#0b1013] px-5 py-5 lg:hidden">{nav.map((item) => <NextLink key={item.href} href={localizedPath(locale, item.href)} className="border-b border-white/10 py-4 text-sm font-bold uppercase tracking-wider">{resolveLocalizedText(item.label, locale)}</NextLink>)}<NextLink href={localizedPath(locale, action.href)} className="mt-5 bg-lime-300 px-5 py-4 text-center text-sm font-black uppercase text-black">{resolveLocalizedText(action.label, locale)}</NextLink></nav>}
    </header>
  );
}
