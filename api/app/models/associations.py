...
class ProductApplicationLink(SQLModel, table=True):
    ...
    product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
    application_id: uuid.UUID = Field(foreign_key="applications.id", ondelete="CASCADE", primary_key=True)


class ProductCertificationLink(SQLModel, table=True):
    __tablename__ = "product_certification_links"
    product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
    certification_id: uuid.UUID = Field(foreign_key="certifications.id", ondelete="CASCADE", primary_key=True)


class ProductFAQLink(SQLModel, table=True):
    __tablename__ = "product_faq_links"
    product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
    faq_item_id: uuid.UUID = Field(foreign_key="faq_items.id", ondelete="CASCADE", primary_key=True)
...
class ProductComparisonLink(SQLModel, table=True):
    ...
    product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
    comparison_topic_id: uuid.UUID = Field(foreign_key="comparison_topics.id", ondelete="CASCADE", primary_key=True)
...
class AlternativePartLink(SQLModel, table=True):
    ...
    product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
    alternative_product_id: uuid.UUID = Field(foreign_key="products.id", ondelete="CASCADE", primary_key=True)
...
class ApplicationRelatedLink(SQLModel, table=True):
    ...
    application_id: uuid.UUID = Field(foreign_key="applications.id", ondelete="CASCADE", primary_key=True)
    related_application_id: uuid.UUID = Field(foreign_key="applications.id", ondelete="CASCADE", primary_key=True)
