...
# ── Public: Download Gate (2.1.5) ──────────────────────────────────────────────

class DownloadGateIn(BaseModel):
    """Minimal lead capture before a gated document download."""
    full_name: str
    email: EmailStr
    company_name: Optional[str] = None
    asset_id: str  # UUID string of the ContentAsset
...
@forms_router.post("/download-gate")
async def submit_download_gate(
    body: DownloadGateIn,
    request: Request,
    db: AsyncSession = Depends(get_session),
):
    """
    2.1.5 Download Gate — visitor provides email/name to unlock a gated document.
    - Creates or updates Contact record.
    - Fires spec_download tracking event.
    ...
    asset = await db.get(ContentAsset, asset_uuid)
    if not asset:
    ...
    # Dedup / upsert Contact by email
    now = utcnow_naive()
    contact = (await db.exec(select(Contact).where(Contact.email == body.email))).first()
    ...
    # Fire spec_download event if visitor_id provided
    if visitor_id_parsed:
        try:
            from app.api.v1.endpoints.events import _upsert_visitor
            async with db.begin_nested():
                await _upsert_visitor(
                    visitor_id_parsed, "spec_download",
                    {"asset_id": str(asset_uuid), "asset_type": asset.asset_type},
                    db,
                )
        except Exception:
            pass
