...
async def trigger_agentOS_content(brief_id: uuid.UUID, tenant_id: Optional[uuid.UUID] = None) -> Optional[str]:
    """
    Trigger AgentOS to start the AI content governance workflow for a PageBrief.

    Returns:
        The AgentOS run_id if successful, None if AgentOS is unavailable (caller handles gracefully).
    """
    from app.models.page_brief import PageBrief

    agentOS_url = settings.AGENTOSS_URL
    if not agentOS_url:
        logger.warning("AGENTOSS_URL not configured, skipping content trigger")
        return None

    task_payload = {
        "tenant_id": str(tenant_id) if tenant_id else "system",
        "domain": "forgebase_ai_content",
        "objective": f"Review and publish content for brief {brief_id}",
        "idempotency_key": str(brief_id),
        "workflow_input": {
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            response_data = response.json()
            run_id = response_data.get("run", {}).get("id")

            if run_id:
                async with get_session_ctx() as db:
                    brief = await db.get(PageBrief, brief_id)
                    if brief:
                        brief.agent_run_id = run_id
                        db.add(brief)
                        await db.commit()

                logger.info(f"PageBrief {brief_id} triggered on AgentOS with run_id={run_id}")
                return run_id
            else:
                logger.error(f"AgentOS response missing run_id for brief {brief_id}: {response_data}")
                return None

    except httpx.HTTPError as e:
        logger.error(f"Failed to trigger AgentOS for brief {brief_id}: {e}")
        raise


async def writeback_agentOS_content_result(brief_id: uuid.UUID, run_id: str, tenant_id: Optional[uuid.UUID] = None) -> bool:
    """
    Write AgentOS run evidence back to the PageBrief record.

    Extracts forgebase_review_content evidence → agent_approved_content_json.
    Idempotent: safe to call multiple times.

    Returns:
        True if writeback succeeded, False if evidence not yet available.
    """
    from app.models.page_brief import PageBrief

    agentOS_url = settings.AGENTOSS_URL
    if not agentOS_url:
        logger.warning("AGENTOSS_URL not configured, skipping content writeback")
        return False
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if response.status_code == 404:
                logger.info(f"AgentOS run {run_id} evidence not yet available for brief {brief_id}")
                return False
            response.raise_for_status()
            evidence_list = response.json()

        approved_content_json: Optional[str] = None

        for ev in evidence_list:
            source_uri = ev.get("source_uri", "")
            payload = ev.get("payload", {})
            output = payload.get("output", {})
            if source_uri == "forgebase_review_content":
                review_packet = output.get("review_packet", {})
                approved_content = review_packet.get("approved_content")
                if approved_content is not None:
                    import json as _json
                    approved_content_json = _json.dumps(approved_content)

        async with get_session_ctx() as db:
            brief = await db.get(PageBrief, brief_id)
            if not brief:
                logger.warning(f"PageBrief {brief_id} not found for content writeback")
                return False

            if approved_content_json is not None:
                brief.agent_approved_content_json = approved_content_json
            db.add(brief)
            await db.commit()

        logger.info(f"PageBrief {brief_id}: content writeback from run {run_id} complete")
        return True

    except httpx.HTTPError as e:
        logger.error(f"Failed to writeback AgentOS content result for brief {brief_id}: {e}")
        raise
