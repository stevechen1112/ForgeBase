...
CONTAINER_SCRIPT = Path("/deploy/configure-tenant-domain-env.py")
REPOSITORY_SCRIPT = (
    Path(__file__).resolve().parents[2] / "deploy" / "configure-tenant-domain-env.py"
)
SCRIPT = CONTAINER_SCRIPT if CONTAINER_SCRIPT.is_file() else REPOSITORY_SCRIPT
...
def test_configure_env_is_idempotent_and_preserves_existing_secret(tmp_path: Path):
    ...
    assert first == second
    assert first["TENANT_ROUTING_SECRET"] == "existing-secret-with-more-than-forty-three-characters-0001"  # pragma: allowlist secret -- test fixture
