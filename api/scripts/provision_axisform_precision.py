...
OWNER_EMAIL = "demo.axisform@forgebase.com"
...
SITE_COPY: dict[str, Any] = {
    ...
    "about": {
        ...
        "timeline": [{"year": "01", "event": "Template selected and adapted to the ForgeBase content model."}, {"year": "02", "event": "Tenant-specific content, assets and domain connected."}, {"year": "03", "event": "Visitor, chat, RFQ and handoff isolation verified."}],
...
async def provision() -> None:
    async with AsyncSessionLocal() as session:
        ...
        owner = await _first(session, User, email=OWNER_EMAIL)
        if owner is None:
            owner = User(tenant_id=tenant.id, email=OWNER_EMAIL, full_name="AxisForm Demo Owner", hashed_password=get_password_hash(secrets.token_urlsafe(48)), role="owner", is_active=True)
        else:
        ...
        profile_values = dict(brand_name="AxisForm Precision", logo_mark="AF", theme_key="precision", layout_key="precision", contact_email="demo@example.com", contact_phone="+00 000 000 000", site_url=SITE_URL, default_locale="en", asset_base=ASSET_BASE, demo_company_folder="precision-machining",
            ...
            site_copy_json=json.dumps(SITE_COPY, ensure_ascii=False), ops_config_json=json.dumps({"auto_reply_enabled":False,"sla_acceptance_hours":24}))
        ...
        for key,headline,sub,label in [("drawing-rfq","Start with the part, not a generic form","Send material, tolerance, volume and target timing.","Send a drawing"),("advisor-rfq","Need help shaping the requirement?","Ask the advisor before opening an RFQ.","Ask the advisor")]:
            await _upsert(session,CTA,{"tenant_id":tenant.id,"cta_key":key,"locale":"en"},{"cta_type":"banner","headline":headline,"subheadline":sub,"button_label":label,"button_action":"open_rfq","button_url":"/rfq","bg_color":"#b8ef36","status":"published"})
